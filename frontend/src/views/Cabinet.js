...
function Cabinet({ token }) {
    function changeEmail() {
        ...
        if (!validEmail) {
            document.getElementById('emailMessage').innerText = "Mistaken data"
            return
        }
    ...
    return (
        <div className="Cabinet">
            ...
            <p id='showEmail'>Your E-Mail: {jwtDecode(token).email}</p>
            <input id='email' placeholder='E-Mail' type='email' />
            <button id='sendEmail' onClick={changeEmail}>Change E-Mail</button>
            <p id='emailMessage'></p>
            <input id='pass' placeholder='Password' type='password' />
            <button id='sendPass' onClick={changePass}>Change password</button>
            <p id='passMessage'></p>
        </div>
    );
}
